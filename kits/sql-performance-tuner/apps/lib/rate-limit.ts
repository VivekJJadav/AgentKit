...
async function checkRedisRateLimit(key: string, signal?: AbortSignal): Promise<RateLimitResult> {
  const url = process.env.UPSTASH_REDIS_REST_URL?.replace(/\/$/, "");
  const token = process.env.UPSTASH_REDIS_REST_TOKEN;
  if (!url || !token) throw new RateLimitConfigurationError("Distributed rate limiting is not configured.");
